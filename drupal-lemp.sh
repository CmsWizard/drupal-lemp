...
echo "==============================================================="
echo "============== Welcome to the Drupal CMS installer ============"
echo "==============================================================="
...
echo "================================================================"
echo "======== A robot is now installing Drupal CMS for you ======="
echo "========================== ETC = 120s =========================="
echo "================================================================"
...
if $varnaked;
  then sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/drupal-lemp/main/config.txt' -O /etc/nginx/sites-enabled/"$var01" && sudo sed -i "s/domain/$var01/g" /etc/nginx/sites-enabled/"$var01";
elif $varwww;
  then sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/drupal-lemp/main/config-www.txt' -O /etc/nginx/sites-enabled/"$var01" && sudo sed -i "s/domain/$var011/g" /etc/nginx/sites-enabled/"$var01";
else 
  sudo wget --no-check-certificate 'https://raw.githubusercontent.com/cmswizard/drupal-lemp/main/config-non-www.txt' -O /etc/nginx/sites-enabled/"$var01" && sudo sed -i "s/domain/$var01/g" /etc/nginx/sites-enabled/"$var01";
fi
...
# installing the app
cd /tmp
wget https://www.drupal.org/download-latest/tar.gz -O latest.tar.gz
mkdir "$var01" && tar -xvzf latest.tar.gz -C "$var01" --strip-components 1
cd
...
echo "=======================-== DONE! ============================"
echo "============ Drupal CMS is installed sucessfully ============"
echo "=================== =================== ====================="
